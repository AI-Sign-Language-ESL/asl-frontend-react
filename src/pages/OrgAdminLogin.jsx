  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data?.requires_2fa) {
        setError('2FA not supported for org admin login');
        return;
      }
      ...
      if (data?.user?.role === 'organization') {
        ...
        setError('Access denied. Organization role required.');
      }
    } catch (err) {
    ...
    <div className="min-h-screen w-full flex items-center justify-center p-6 bg-bg-main">
      <div className="w-full max-w-md glass rounded-2xl border border-border-subtle p-8 relative z-10">
        <div className="mb-8 text-center">
          <h1 className="text-3xl font-bold text-text-main">Organization Admin Login</h1>
          <p className="text-text-muted mt-2">Enter your credentials to access your organization dashboard</p>
        </div>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                placeholder="org@tafahom.io"
                required
                disabled={loading}
                className="w-full bg-bg-card border border-border-subtle rounded-xl pl-12 pr-4 py-3.5 text-text-main text-sm disabled:opacity-50"
              />
            </div>
          </div>
